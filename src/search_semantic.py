...
def hybrid_rerank(cands: List[Dict], query: str, alpha: float = 0.7, fuzzy_threshold: int = 85) -> List[Dict]:
    out = []
    for c in cands:
...
def dedup_by_paper(items: List[Dict]) -> List[Dict]:
    ...
    for _, arr in groups.items():
        best = max(arr, key=lambda x: (x.get("hybrid_score", -1e9), x.get("score", -1e9)))
        merged_count = len(arr) - 1
        best = dict(best)
        best["merged_passages"] = merged_count
        deduped.append(best)

    deduped.sort(key=lambda x: (x.get("hybrid_score", -1e9), x.get("score", -1e9)), reverse=True)
    return deduped
...
def main():
    ...
    index = load_index()
    meta = load_meta()
    model = SentenceTransformer(cfg.get("model", "sentence-transformers/all-MiniLM-L6-v2"))
    ...
    while True:
        ...
        for rank, r in enumerate(results, 1):
            ...
            print(f"    ⭐ cos={r.get('score',0):.4f} • kw={r.get('kw_overlap',0):.2f} • hybrid={r.get('hybrid_score',0):.4f} • merged_passages={r.get('merged_passages',0)}")
